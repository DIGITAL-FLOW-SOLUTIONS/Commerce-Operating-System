...
const STATUS_CONFIG: Record<string, { label: string; color: string; icon: typeof Package }> = {
  pending: { label: "Awaiting payment", color: "text-yellow-600 bg-yellow-50", icon: Clock },
  paid: { label: "Payment confirmed", color: "text-emerald-700 bg-emerald-50", icon: CheckCircle2 },
  processing: { label: "Processing", color: "text-blue-700 bg-blue-50", icon: Package },
  shipped: { label: "On the way", color: "text-purple-700 bg-purple-50", icon: Truck },
  delivered: { label: "Delivered", color: "text-emerald-700 bg-emerald-50", icon: CheckCircle2 },
  failed: { label: "Payment failed", color: "text-red-700 bg-red-50", icon: Clock },
};
...
export default function SocialTrackPage() {
  ...
  const demoOrders = searchPhone ? [{ id: "demo-order-so", phone: searchPhone, totalKes: 1800, status: "paid", items: [{ productName: "Vitamin C Brightening Serum", quantity: 1, unitPriceKes: 1800, totalKes: 1800, productId: "so-p1" }], createdAt: new Date(Date.now() - 7200000).toISOString() }] : [];
  const displayOrders = isDemo ? demoOrders : (orders ?? []);

  return (
    <div className="min-h-screen bg-background">
      <StoreHeader storeName="Track Order" showBack backTo={`/social/${storeId}`} compact />
      <div className="px-4 py-6 max-w-lg mx-auto">
        ...
        {searchPhone && !isLoading && displayOrders.length === 0 && <EmptyState icon={<Package size={24} />} title="No orders found" description="No orders associated with this number." />}
